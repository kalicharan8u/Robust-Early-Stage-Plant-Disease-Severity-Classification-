...
# Enhanced Quokka Swarm Optimization (EQSO) Starting Line No. 36
def Proposed(population, objective_function, lb, ub, max_iter):
    ...
    for iteration in range(max_iter):
        ...
        for i in range(pop_size):
            currentfit = fitness_values[i]
            bestfit = np.min(fitness_values)
            worstfit = np.max(fitness_values)
            r = currentfit + bestfit / (worstfit ** 2)
